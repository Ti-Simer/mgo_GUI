...
@Component({
  selector: 'app-reports-range',
  templateUrl: './reports-range.component.html',
  styleUrls: ['./reports-range.component.scss']
})
export class ReportsRangeComponent {
  ...
  constructor(
    ...
    this.route.params.subscribe(params => {
      ...
      console.log(this.branchOfficeCode);
  ...
  fetchData() {
    ...
    this.graphService.generateCsvByBranchOffice(this.branchOfficeCode, this.reportXlsMonthForm.value).subscribe(
      response => {
        ...
        if (response.statusCode == 200) {
          ...
          for (let i = 0; i < this.records.length; i++) {
            ...
            // Calcular el promedio de total
            this.promedioTotal =
              this.records.reduce(
                (total: any, record: any) => total + parseFloat(record.total),
                0
              ) / this.records.length
          }
          ...
          if (this.records) {
            ...
            this.groupedRecords = this.records.reduce((acc, record) => {
              ...
              if (record.status === 'EFECTIVO') {
                acc[record.plate].efectivo.count += 1;
                acc[record.plate].efectivo.kilos += record.masaTotal;
              } else if (record.status === 'PENDIENTE') {
                acc[record.plate].pendiente.count += 1;
                acc[record.plate].pendiente.kilos += record.masaTotal;
              } else if (record.status === 'ERROR') {
                acc[record.plate].error.count += 1;
                acc[record.plate].error.kilos += record.masaTotal;
              }

              acc[record.plate].total_cilindros += 1;
              acc[record.plate].total_kilos += record.masaTotal;
  ...
  drawChartBar() {
    setTimeout(() => {
      ...
      let labels = [...new Set(this.records.map((record: { plate: string }) => record.plate))];
      let data = labels.map(label => this.records.filter((record: { plate: string }) => record.plate === label).length);
      ...
      switch (this.languageService.getLanguage()) {
        case 'es':
          labelText = ` Cargues por auto-tanque`;
          break;

        case 'en':
          labelText = ` Loads per propane truck`;
          break;

        case 'pt':
          labelText = ` Cargas por auto-tanque`;
          break;
