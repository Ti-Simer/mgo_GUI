...
export class LogReportListMapComponent {
  ...
  receiveData() {
    this.logReportService.getLastLogsByPlate().subscribe(
      reponse => {
          ...
          if (this.data) {
            ...
            this.formattedData = this.data
              ...
              .map((item: any) => ({
                ...
                route_event: item.route_event.name,
                code_event: item.route_event.code_event,
                date: item.create,
              }));

            this.mapCenter = {
              lat: parseFloat(this.formattedData[0].latitude),
              lng: parseFloat(this.formattedData[0].longitude)
            };

            console.log('Formatted data:', this.formattedData);

            // Itera a través de los waypoints y crea los marcadores y modos de transporte
            for (const location of this.formattedData) {
